...
(POST /public/agents, source='tracent', submitted_by set), which must stay
...
async def test_submit_requires_no_auth_and_creates_pending_agent(client):
    ...
    assert result["tracent_id"].startswith("trc_")
...
async def test_pending_agent_absent_from_legacy_list_agents(client, master_key_headers):
    ...
    assert result["tracent_id"] not in ids

    search_resp = await client.get(
        "/agents", params={"q": "Ghost Agent Legacy"}, headers=master_key_headers
    )
    assert result["tracent_id"] not in {a["tracent_id"] for a in search_resp.json()["agents"]}
...
async def test_pending_agent_excluded_from_both_categories_surfaces(client, master_key_headers):
    ...
    tracent_id = result["tracent_id"]

    async with db_module.get_conn() as conn:
        await insert_skill(conn, tracent_id=tracent_id, skill_name="ghost-skill", tags=["ghost-category"])
...
async def test_pending_agent_visible_via_direct_id_lookup_on_both_surfaces(client, master_key_headers):
    ...
    legacy = await client.get(f"/agents/{tracent_id}", headers=master_key_headers)
    assert legacy.status_code == 200
    assert legacy.json()["moderation_status"] == "pending"

    public = await client.get(f"/public/agents/{tracent_id}")
...
async def test_admin_submissions_lists_pending_with_submitter_email(client, master_key_headers):
    ...
    submissions = {s["tracent_id"]: s for s in resp.json()["submissions"]}
    assert result["tracent_id"] in submissions
    assert submissions[result["tracent_id"]]["submitter_email"] == "findme@example.com"
    assert submissions[result["tracent_id"]]["moderation_status"] == "pending"
...
async def test_submitter_email_and_note_never_leak_while_pending(client, master_key_headers):
    ...
    for path in (f"/agents/{tracent_id}", f"/public/agents/{tracent_id}"):
...
async def test_submitter_email_and_note_never_leak_after_approval(client, master_key_headers):
    ...
    approve = await client.post(
        f"/admin/submissions/{tracent_id}/review",
    ...
    legacy_detail = await client.get(f"/agents/{tracent_id}", headers=master_key_headers)
    assert "submitter_email" not in legacy_detail.json()
    assert "moderation_note" not in legacy_detail.json()

    public_detail = await client.get(f"/public/agents/{tracent_id}")
...
async def test_approving_makes_agent_appear_on_both_surfaces(client, master_key_headers):
    ...
    assert tracent_id not in {a["tracent_id"] for a in before_legacy.json()["agents"]}
    before_public = await client.get("/public/agents")
    assert tracent_id not in {a["tracent_id"] for a in before_public.json()["agents"]}
    ...
    assert tracent_id in {a["tracent_id"] for a in after_legacy.json()["agents"]}
    after_public = await client.get("/public/agents")
    assert tracent_id in {a["tracent_id"] for a in after_public.json()["agents"]}
...
async def test_rejecting_keeps_agent_hidden_on_both_surfaces(client, master_key_headers):
    ...
    assert tracent_id not in {a["tracent_id"] for a in legacy.json()["agents"]}
    public = await client.get("/public/agents")
    assert tracent_id not in {a["tracent_id"] for a in public.json()["agents"]}
...
async def test_reviewing_unknown_tracent_id_returns_404(client, master_key_headers):
...
async def test_review_requires_auth(client):
    result = await _submit(client)
    resp = await client.post(
        f"/admin/submissions/{result['tracent_id']}/review",
        json={"action": "approve"},
    )
    assert resp.status_code == 401
...
async def test_approved_submission_disappears_from_admin_queue(client, master_key_headers):
    ...
    assert tracent_id in {s["tracent_id"] for s in pending_list.json()["submissions"]}
    ...
    assert tracent_id not in {s["tracent_id"] for s in after_list.json()["submissions"]}
...
async def test_reject_does_not_touch_a_different_pending_agents_source_or_moderation(client, master_key_headers):
    ...
    await client.post(
        f"/admin/submissions/{reject['tracent_id']}/review",
        json={"action": "reject"},
        headers=master_key_headers,
    )

    still_pending = await client.get("/admin/submissions", headers=master_key_headers)
    ids = {s["tracent_id"] for s in still_pending.json()["submissions"]}
    assert keep["tracent_id"] in ids
    assert reject["tracent_id"] not in ids
...
async def test_contribute_flow_is_instant_live_with_no_moderation_gate(client, jwt_user):
    ...
    # Unlike POST /agents/submit, this is source='tracent' and carries no
...
async def test_contribute_flow_agent_is_immediately_visible_on_legacy_surface_too(
    ...
    resp = await client.post("/public/agents", json=LISTING_BODY, headers=auth_headers)
    assert resp.status_code == 200, resp.text
    tracent_id = resp.json()["tracent_id"]

    legacy_list = await client.get("/agents", headers=master_key_headers)
    assert tracent_id in {a["tracent_id"] for a in legacy_list.json()["agents"]}
    ...
    assert tracent_id not in {s["tracent_id"] for s in submissions.json()["submissions"]}
