...
safe_to_transact, reputation_flags) onto a small fixed set of internal
tokens: "tracent_verified", "tracent_hosted", "verified", "flagged",
"unverified". These are backend-internal identifiers, not display text —
the frontend translates each token into plain-English copy (e.g.
"Genticspace Verified") via lib/trust.ts's getTrustInfo. Genticspace is the
product's user-facing brand; "tracent" here is purely the backend's own
naming for this trust tier, unrelated to what's shown to end users.

This is a presentation-layer contract: frontend code renders a label per
token with no further interpretation, so the token set and precedence
...
def compute_trust_summary(
    *,
    ...
    Precedence (first match wins):
      ...
      2. "tracent_hosted" — trust_tier == "tracent-hosted": Genticspace wrote
         ...
         "flagged" for the same reason tracent_verified does: Genticspace is
      ...
      5. "unverified" — everything else: trust_tier is null/unset, or the
         ...
         self-contributed listings — which don't go through on-chain
