"""
One-off script: register the "hello-world" Genticspace-hosted agent proof of
...
(source, source_id) upsert key, same genticspace_id generation as
app/services/indexer.py's `_generate_genticspace_id`); deslop doesn't currently
...
row is meant to describe a real (if locally-simulated) running agent, so:
  ...
  - source            = 'genticspace-hosted'  (new source value — nothing else
                         in the indexer writes this; it marks "Genticspace runs
                         this process itself" as opposed to "this endpoint
                         lives somewhere else that Genticspace only tracks")
  - source_id         = 'hello-world'     (the agent's slug — see
                         agents/hello-world/genticspace.yaml)
...
genticspace_id, source, source_id, name, description, is_active, x402_support,
...
Usage:
    ...
    # 3. Verify (with the Genticspace backend running separately):
...
Cleanup:
    DELETE FROM agents WHERE source = 'genticspace-hosted' AND source_id = 'hello-world';
"""
import argparse
import asyncio
import secrets
...
def _generate_genticspace_id() -> str:
    # Mirrors app/services/indexer.py's _generate_genticspace_id exactly, so
    # Genticspace-hosted agents get IDs indistinguishable in shape from
    # externally-discovered ones.
    suffix = secrets.token_urlsafe(10)[:10]
    return f"trc_{suffix}"


# trust_tier: uses the dedicated 'genticspace-hosted' value — distinct from
# 'onchain' (implies ERC-8004 registration; this agent has none: no
# owner_address, no registered_block/tx) and 'genticspace' (paid human review of
...
# did). See app/services/trust_summary.py's "genticspace_hosted" label and
...
INSERT INTO agents (
    ...
    $6, $7, TRUE,
    $8,
    TRUE, 'genticspace-hosted', $9,
    0.0, FALSE,
    NOW()
)
ON CONFLICT (source, source_id) DO UPDATE SET
    genticspace_id       = agents.genticspace_id,
    name             = EXCLUDED.name,
...
async def register(mcp_endpoint: str, web_endpoint: str) -> str:
    ...
    try:
        async with get_conn() as conn:
            genticspace_id_for_insert = _generate_genticspace_id()
            row = await conn.fetchrow(
                _UPSERT_SQL,
                genticspace_id_for_insert,
                "genticspace-hosted",
                "hello-world",
                "Genticspace Hello World",
                "Hard-coded echo agent — proof of concept for Genticspace's first-party agent hosting.",
                mcp_endpoint,
                web_endpoint,
                "Genticspace",
                _NOW,
            )
            genticspace_id = row["genticspace_id"]

            await conn.execute("DELETE FROM agent_skills WHERE genticspace_id = $1", genticspace_id)
            await conn.execute(
                """
                INSERT INTO agent_skills (genticspace_id, skill_id, skill_name, description, tags)
                VALUES ($1, $2, $3, $4, $5)
                """,
                genticspace_id, "echo", "Echo",
...
if __name__ == "__main__":
    ...
    genticspace_id = asyncio.run(register(args.mcp_endpoint, args.web_endpoint))
    print(f"Registered hello-world hosted agent as {genticspace_id} (source=genticspace-hosted)")
    print(f"  mcp_endpoint = {args.mcp_endpoint}")
    print(f"  web_endpoint = {args.web_endpoint}")
    print(f"Verify: curl -H \"X-API-Key: $API_KEY\" http://localhost:8000/agents/{genticspace_id}")
