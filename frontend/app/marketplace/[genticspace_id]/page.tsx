...
import { getAgent, getSandboxConfig, ApiError } from "@/lib/api";
...
export default async function AgentProfilePage({
  ...
  // Track A (HF Space) is known synchronously from agent.sandboxable; Track
  // B (GitHub repo with a genticspace.yaml manifest) needs its own lookup —
  // see components/agent/SandboxSection.tsx and RunConsole.tsx for what
  // each track actually does.
  const isSpaceReady = agent.sandboxable && !!agent.sandbox_url;
  const isRepoReady = !isSpaceReady && (await getSandboxConfig(genticspace_id).catch(() => null))?.sandbox_enabled;
          ...
          <div className="relative">
            ...
            >
              ...
              <div className="flex flex-col gap-6 min-w-0">
                {isSpaceReady && <SandboxSection genticspaceId={agentId(agent)} track="space" />}
                {isRepoReady && <SandboxSection genticspaceId={agentId(agent)} track="repo" />}
